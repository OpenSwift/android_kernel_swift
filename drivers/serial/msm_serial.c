...
#if defined(CONFIG_MACH_MSM7X27_SWIFT)
int __init uart_mode_detect(char* s)
{
	if(!strcmp(s,"arm11_uart_disable"))
		uart_det_flag = 1;
	else if(!strcmp(s,"arm11_uart_enable"))
		uart_det_flag = 0;
	
	return 1;
}
__setup("uart.mode=",uart_mode_detect);
#endif
#ifdef CONFIG_MACH_MSM7X27_SWIFT
fs_initcall(msm_serial_init);
#else
