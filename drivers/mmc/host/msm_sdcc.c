...
msmsdcc_start_command_exec(struct msmsdcc_host *host, u32 arg, u32 c)
{
	...
	if ( readl(host->base + MMCICOMMAND) == 0 ) {
		...
		if (readl(host->base + MMCIARGUMENT) != arg)
			writel(arg, host->base + MMCIARGUMENT);
...
/* Clean up clock management and add a 10us delay after enabling clocks */
    ...
    if (host->plat->status_irq == 113) {
    	...
    	if (status ^ host->oldstat) {
            ...
            if(status) {
                for(i=0; i<5; i++){
                    mdelay(10);
                    printk("%s is called - times: %d\n",__func__,i);
           	        status2 = host->plat->status(mmc_dev(host->mmc));
                    if(status != status2) {
                       return;
                    }
                }
...
static void msmsdcc_early_suspend(struct early_suspend *h)
{
	...
	unsigned long flags;
#ifdef CONFIG_LGE_BCM432X_PATCH
	printk(KERN_ERR "[dongp.kim] [%s:%d] irq:%d\n", __func__, __LINE__, host->plat->status_irq);
...
#ifdef CONFIG_LGE_BCM432X_PATCH
	printk(KERN_ERR "[dongp.kim] [%s:%d] irq:%d\n", __func__, __LINE__, host->plat->status_irq);
	if ( host->plat->status_irq != gpio_to_irq(CONFIG_BCM4325_GPIO_WL_RESET)) /* don't do this to WLAN */
	{
		unsigned int status;
		if (!host->plat->status) {
			mmc_detect_change(host->mmc, 0);
...
/* Fix me: request_irq function invoke handler function without interrupt.
 * It makes detection failure when we boot up device with inserting the micro sd card.
 */
#if defined (CONFIG_MACH_LGE)
...
#endif /* defined(CONFIG_BRCM_LGE_WL_HOSTWAKEUP) && !defined(CONFIG_BRCM_LGE_WL_HWAKEUP_EARLY_SUSPEND) */
		if (!rc) {
			writel(0, host->base + MMCIMASK0);
