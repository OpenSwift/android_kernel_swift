...
/* To use debug message, we can invoke MODEM_TRACE(); */
MODEM_DEBUG_PARAM;
...
struct gs_dev {
	...
	struct usb_interface_descriptor gs_nmea_ifc_desc;
...
static void gs_init_com_ifc_desc(struct usb_interface_descriptor *ifc_desc)
{
	...
	ifc_desc->bInterfaceSubClass =	USB_CDC_SUBCLASS_ACM;
	/* ifc_desc->bInterfaceProtocol =	USB_CLASS_VENDOR_SPEC; */
	ifc_desc->bInterfaceProtocol =	USB_CDC_ACM_PROTO_AT_V25TER;
...
static void gs_init_nmea_ifc_desc(struct usb_interface_descriptor *ifc_desc)
{
	ifc_desc->bLength =		USB_DT_INTERFACE_SIZE;
	ifc_desc->bDescriptorType =	USB_DT_INTERFACE;
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
static void gs_init_nmea_header_desc(struct gs_dev *dev)
{
	/* For initializing GPS header, we use com interface structure */
	dev->gs_highspeed_header[0] =
		(struct usb_descriptor_header *)&dev->gs_nmea_ifc_desc;
	dev->gs_highspeed_header[1] =
		(struct usb_descriptor_header *)&dev->gs_hs_bulkin_desc;
	dev->gs_highspeed_header[2] =
		(struct usb_descriptor_header *)&dev->gs_hs_bulkout_desc;
...
/* 2009-11-30, Add codes for allocating header of GPS(NMEA) separately */
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
		if (i == 0) { /* For initializing Modem */
			/*6 Interfaces, 3 Endpoints-> Total 9 + 1 for NULL*/
			...
			gs_init_nmea_ifc_desc(&gs_devices[i]->gs_nmea_ifc_desc);
		}
#else /* Below is original */
		/*1 - Interface, 3 Endpoints-> Total 4 + 1 for NULL*/
		gs_devices[i]->gs_fullspeed_header =
		...
		if (i == 0) /* For modem */
			...
			gs_init_nmea_header_desc(gs_devices[i]);
#else /* Below is original */
		gs_init_header_desc(gs_devices[i]);
#endif  /* CONFIG_MACH_MSM7X27_SWIFT */
...
//	MODEM_TRACE(MODEM_DEBUG_INIT, "USB serial init called\n");
	return 0;
}
		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	gs_debug("gs_ioctl: (%d,%p,%p) cmd=0x%4.4x, arg=0x%lx\n",
...
static void gs_bind(void *_ctxt)
{
	...
	if (!strcmp(func->name, "modem")) {
		...
		ret = usb_msm_get_next_ifc_number(func);
		dev->gs_nmea_ifc_desc.bInterfaceNumber = ret;
		dev->gs_nmea_ifc_desc.iInterface = 0;
	}
#else /* Below is original */
	ret = usb_msm_get_next_ifc_number(func);
