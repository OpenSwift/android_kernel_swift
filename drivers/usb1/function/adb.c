...
/* To use debug message, we can invoke ADB_TRACE(); */
ADB_DEBUG_PARAM;


/* From Google's msm project(android-msm.git), patch that queue read
 * requests with length specified by client is merged. This #define is
 * for the patch.
 *
 * references:
 * http://r.android.com/#change,12222
 * http://r.android.com/#change,12245
 */
#define LGE_GOOGLE_PATCH
...
/* This is BIG part of the patches */
#if defined(LGE_GOOGLE_PATCH)
requeue_req:
	/* queue a request */
	req = ctxt->rx_req;
...
fail:
#endif /* End of defined(LGE_GOOGLE_PATCH) */
	_unlock(&ctxt->read_excl);
	return r;
}
