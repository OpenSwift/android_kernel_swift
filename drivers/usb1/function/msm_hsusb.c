...
/* This is for solving DUN service problem */
...
/* To reduce probe() call time */
/* #define LGE_SAVE_BOOT_TIME */
#undef LGE_SAVE_BOOT_TIME
/* To detect Travel Adapter */
#define LGE_DETECT_TA
/* #undef LGE_DETECT_TA */
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT)
#define LGE_PIF_DETECT 2
#endif  /* CONFIG_MACH_MSM7X27_SWIFT */

/* As error and info messages use pr_err() and pr_info() respectivly,
 * provided in linux kernel, We use following macro for only debugging.
 *
 * 2009-11-17, Add debug mask to make debugging message enable/disable in
 * runtime
 */
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(LGE_HSUSB_DEBUG_PRINT)
...
#define LGE_INIT_PID        0x6003
#define LGE_UMS_ONLY_PID        0x61B4 /* UMS only */ 
#define LGE_ANDROID_PID     0x618E

static int pid = LGE_INIT_PID;   /* LGE default product ID (currently) */
...
static int usb_chg_detect_type(struct usb_info *ui)
{
	...
	{
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(LGE_DETECT_TA)
		if (msm_hsusb_detect_chg_type() == CHG_WALL)
			ret = CHG_WALL;
		else
			ret = CHG_HOST_PC;
   		break;
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
static void lge_line_coding_complete(struct usb_endpoint *ept, struct usb_request *req)
{
	struct usb_info *ui = the_usb_info;
	req->complete = 0;
	ep0_setup_ack(ui);
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
				if (ctl.bRequest == 0x20) { /* USB_CDC_REQ_SET_LINE_CODING */
					req->length = 7;
					req->complete = lge_line_coding_complete;
					usb_ept_queue_xfer(&ui->ep0out, req);
					return;
				}
#endif  /* CONFIG_MACH_MSM7X27_SWIFT */
...
int lge_detect_pif(void)
{
	...
	return ui->pif_detect;
}
EXPORT_SYMBOL(lge_detect_pif);
#endif  /* CONFIG_MACH_MSM7X27_SWIFT */
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
	if (ui->pif_detect == LGE_PIF_DETECT) {
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
static inline int check_lge_product(int vid, int pid)
{
	if (vid != LGE_VENDOR_ID)
		return 0;

	if ((pid == LGE_ANDROID_PID) || (pid == LGE_INIT_PID) ||
		(pid == LGE_FACTORY_PID))
		return pid;
...
/* To support LG USB Host driver, if product id is in LGE product id set,
 * device class is changed to 0x02 (CDC class).
 *
 * 2009-12-31, If pid is 0x6171, as this pid uses QCT driver, bDeviceClass
 * and bDeviceSubClass must be 0 and 0 respectively.
 *
 * 2010-02-11, For New Android Host driver, QCT driver's DeviceClass is
 * no more needed.
 */
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
	if (check_lge_product(desc_device.idVendor, desc_device.idProduct)) {
...
#if defined(CONFIG_MACH_MSM7X27_SWIFT) && defined(CONFIG_LGE_USB_DRIVER)
	if (desc_device.idProduct == LGE_INIT_PID)
		desc_device.idProduct = LGE_ANDROID_PID;
#endif  /* CONFIG_MACH_MSM7X27_SWIFT */
	 ...
	 * (MODEM+DIAG+GPS), It may happens problem that cannot make phone
	 * on factory line procedure. Due to this problem, we set serial_number
	 * to NULL in case of LGE factory mode. For other cases, we set
	 * serial_number to IMEI.
	 */
	switch(desc_device.idProduct) {
		case LGE_INIT_PID :
		case LGE_FACTORY_PID :
			ui->pdata->serial_number = NULL;
			break;
		case LGE_UMS_ONLY_PID :
		case LGE_ANDROID_PID :
		default :
			if (nv_imei_ptr[0] != '\0') {
				if ((nv_imei_ptr[0] == '8') && 
					(nv_imei_ptr[1] == '0') && 
	...
	if (ui->pdata->manufacturer_name)
		desc_device.iManufacturer =
			usb_msm_get_next_strdesc_id(ui->pdata->manufacturer_name);
...
static int __init usb_probe(struct platform_device *pdev)
{
	...
	msm_proc_comm(PCOM_CUSTOMER_CMD2, &ui->pif_detect, &cmd_pif);
