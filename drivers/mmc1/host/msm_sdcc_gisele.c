...
msmsdcc_check_status(unsigned long data)
{
	...
	printk(KERN_ERR "[host->plat->status_irq: %d:%d]\n", host->plat->status_irq, __LINE__);
	printk(KERN_ERR "[MSM_GPIO_TO_INT(CONFIG_BCM4329_GPIO_WL_RESET): %d:%d]\n", MSM_GPIO_TO_INT(CONFIG_BCM4329_GPIO_WL_RESET), __LINE__);
	...
	if (!host->plat->status) {
		...
		if (status ^ host->oldstat) {
			...
			if ((host->plat->status_irq == 94) && status) { /* don't do this to WLAN */  
				mmc_detect_change(host->mmc, (3 * HZ) / 2);
#ifdef CONFIG_LGE_BCM432X_PATCH
				printk(KERN_ERR "out[%s:%d]\n", __func__, __LINE__);
			}
			else if ((host->plat->status_irq == MSM_GPIO_TO_INT(CONFIG_BCM4329_GPIO_WL_RESET)) && status) { /* do this to BCM4329 */ 
				printk(KERN_ERR "in[%s:%d]\n", __func__, __LINE__);
				mmc_detect_change(host->mmc, (3 * HZ) / 20); //150msec
			}
#endif /* CONFIG_LGE_BCM432X_PATCH */
			else {
				printk(KERN_ERR "[%s:%d]\n", __func__, __LINE__);
