/* arch/arm/mach-msm/board-griffin-fuel.c
 *
...
static void max17040_init_chip(void)
{
	...
	max17040_write_reg(max17040_dev->client, 0x06, 0x40);
	max17040_write_reg(max17040_dev->client, 0x07, 0x00);
...
static void max17040_work_func(struct work_struct *work)
{
	...
	if (!i2c_check_functionality(max17040_dev->client->adapter, I2C_FUNC_I2C)) {
		...
		schedule_delayed_work(&max17040_dev->work, HZ/10);
...
static int __init max17040_probe(struct i2c_client *i2c_dev, const struct i2c_device_id *i2c_dev_id)
{
	...
	device_create_file(&i2c_dev->dev, &dev_attr_current_vol);
	device_create_file(&i2c_dev->dev, &dev_attr_current_percent);
...
MODULE_DESCRIPTION("Griffin Fuel Guage Driver");
